...
pub struct QueryIter<'w, 's, D: QueryData, F: QueryFilter, C: QueryCache> {
    world: UnsafeWorldCell<'w>,
    tables: &'w Tables,
    archetypes: &'w Archetypes,
    query_state: &'s QueryState<D, F, C>,
    cursor: QueryIterationCursor<'w, 's, D, F, C>,
}

impl<'w, 's, D: QueryData, F: QueryFilter, C: QueryCache> QueryIter<'w, 's, D, F, C> {
    /// # Safety
    ...
    /// }
    /// ```
    pub fn remaining(&self) -> QueryIter<'w, 's, D, F, C>
    where
        D: ReadOnlyQueryData,
    {
        QueryIter {
    ...
    /// }
    /// ```
    pub fn remaining_mut(&mut self) -> QueryIter<'_, 's, D, F, C> {
        QueryIter {
            world: self.world,
            tables: self.tables,
            archetypes: self.archetypes,
...
impl<'w, 's, D: QueryData, F: QueryFilter, C: QueryCache> Iterator for QueryIter<'w, 's, D, F, C> {
...
// SAFETY: [`QueryIter`] is guaranteed to return every matching entity once and only once.
unsafe impl<'w, 's, F: QueryFilter, C: QueryCache> EntitySetIterator
    for QueryIter<'w, 's, FilteredEntityRef<'_, '_>, F, C>
{
}

// SAFETY: [`QueryIter`] is guaranteed to return every matching entity once and only once.
unsafe impl<'w, 's, F: QueryFilter, C: QueryCache> EntitySetIterator
    for QueryIter<'w, 's, FilteredEntityMut<'_, '_>, F, C>
{
}

// SAFETY: [`QueryIter`] is guaranteed to return every matching entity once and only once.
unsafe impl<'w, 's, F: QueryFilter, B: Bundle, C: QueryCache> EntitySetIterator
    for QueryIter<'w, 's, EntityRefExcept<'_, '_, B>, F, C>
{
}

// SAFETY: [`QueryIter`] is guaranteed to return every matching entity once and only once.
unsafe impl<'w, 's, F: QueryFilter, B: Bundle, C: QueryCache> EntitySetIterator
    for QueryIter<'w, 's, EntityMutExcept<'_, '_, B>, F, C>
{
}

impl<'w, 's, D: QueryData, F: QueryFilter, C: QueryCache> Debug for QueryIter<'w, 's, D, F, C> {
...
pub struct QuerySortedIter<'w, 's, D: QueryData, F: QueryFilter, C: QueryCache, I>
where
...
impl<'w, 's, D: QueryData, F: QueryFilter, C: QueryCache, I: Iterator> QuerySortedIter<'w, 's, D, F, C, I>
where
    I: Iterator<Item = Entity>,
{
    /// # Safety
...
impl<'w, 's, D: QueryData, F: QueryFilter, C: QueryCache, I: Iterator> Iterator
    for QuerySortedIter<'w, 's, D, F, C, I>
where
    I: Iterator<Item = Entity>,
{
    type Item = D::Item<'w, 's>;
...
impl<'w, 's, D: QueryData, F: QueryFilter, C: QueryCache, I: Iterator> DoubleEndedIterator
    for QuerySortedIter<'w, 's, D, F, C, I>
where
...
impl<'w, 's, D: QueryData, F: QueryFilter, C: QueryCache, I: Iterator> ExactSizeIterator
    for QuerySortedIter<'w, 's, D, F, C, I>
where
...
impl<'w, 's, D: QueryData, F: QueryFilter, C: QueryCache, I: Iterator> FusedIterator
    for QuerySortedIter<'w, 's, D, F, C, I>
where
...
unsafe impl<'w, 's, F: QueryFilter, C: QueryCache, I: Iterator<Item = Entity>> EntitySetIterator
    for QuerySortedIter<'w, 's, Entity, F, C, I>
{
}

impl<'w, 's, D: QueryData, F: QueryFilter, C: QueryCache, I: Iterator<Item = Entity>> Debug
    for QuerySortedIter<'w, 's, D, F, C, I>
{
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
...
pub struct QueryManyIter<'w, 's, D: QueryData, F: QueryFilter, C: QueryCache, I: Iterator<Item: EntityEquivalent>>
{
    world: UnsafeWorldCell<'w>,
    entity_iter: I,
    entities: &'w Entities,
...
impl<'w, 's, D: QueryData, F: QueryFilter, C: QueryCache, I: Iterator<Item: EntityEquivalent>>
    QueryManyIter<'w, 's, D, F, C, I>
{
    /// # Safety
    /// - `world` must have permission to access any of the components registered in `query_state`.
    /// - `world` must be the same one used to initialize `query_state`.
...
impl<'w, 's, D: QueryData, F: QueryFilter, C: QueryCache, I: DoubleEndedIterator<Item: EntityEquivalent>>
    QueryManyIter<'w, 's, D, F, C, I>
{
...
impl<'w, 's, D: ReadOnlyQueryData, F: QueryFilter, C: QueryCache, I: Iterator<Item: EntityEquivalent>> Iterator
    for QueryManyIter<'w, 's, D, F, C, I>
{
    type Item = D::Item<'w, 's>;
...
impl<
    ...
    > DoubleEndedIterator for QueryManyIter<'w, 's, D, F, C, I>
{
    #[inline(always)]
    fn next_back(&mut self) -> Option<Self::Item> {
        // SAFETY:
...
impl<'w, 's, D: ReadOnlyQueryData, F: QueryFilter, C: QueryCache, I: Iterator<Item: EntityEquivalent>>
    FusedIterator for QueryManyIter<'w, 's, D, F, C, I>
{
}

// SAFETY: Fetching unique entities maintains uniqueness.
unsafe impl<'w, 's, F: QueryFilter, C: QueryCache, I: EntitySetIterator> EntitySetIterator
    for QueryManyIter<'w, 's, Entity, F, C, I>
{
}

impl<'w, 's, D: QueryData, F: QueryFilter, C: QueryCache, I: Iterator<Item: EntityEquivalent>> Debug
    for QueryManyIter<'w, 's, D, F, C, I>
{
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
...
pub struct QueryManyUniqueIter<'w, 's, D: QueryData, F: QueryFilter, C: QueryCache, I: EntitySetIterator>(
    QueryManyIter<'w, 's, D, F, C, I>,
);

impl<'w, 's, D: QueryData, F: QueryFilter, C: QueryCache, I: EntitySetIterator>
    QueryManyUniqueIter<'w, 's, D, F, C, I>
{
    /// # Safety
    /// - `world` must have permission to access any of the components registered in `query_state`.
    /// - `world` must be the same one used to initialize `query_state`.
...
impl<'w, 's, D: QueryData, F: QueryFilter, C: QueryCache, I: EntitySetIterator> Iterator
    for QueryManyUniqueIter<'w, 's, D, F, C, I>
{
    type Item = D::Item<'w, 's>;
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        unsafe {
            QueryManyIter::<'w, 's, D, F, C, I>::fetch_next_aliased_unchecked(
...
// This is correct as [`QueryManyIter`] always returns `None` once exhausted.
impl<'w, 's, D: QueryData, F: QueryFilter, C: QueryCache, I: EntitySetIterator> FusedIterator
    for QueryManyUniqueIter<'w, 's, D, F, C, I>
{
}
...
// SAFETY: Fetching unique entities maintains uniqueness.
unsafe impl<'w, 's, F: QueryFilter, C: QueryCache, I: EntitySetIterator> EntitySetIterator
    for QueryManyUniqueIter<'w, 's, Entity, F, C, I>
{
}

impl<'w, 's, D: QueryData, F: QueryFilter, C: QueryCache, I: EntitySetIterator> Debug
    for QueryManyUniqueIter<'w, 's, D, F, C, I>
{
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
...
pub struct QuerySortedManyIter<'w, 's, D: QueryData, F: QueryFilter, C: QueryCache, I: Iterator<Item = Entity>> {
    entity_iter: I,
    entities: &'w Entities,
    tables: &'w Tables,
    archetypes: &'w Archetypes,
...
impl<'w, 's, D: QueryData, F: QueryFilter, C: QueryCache, I: Iterator<Item = Entity>>
    QuerySortedManyIter<'w, 's, D, F, C, I>
{
    /// # Safety
    /// - `world` must have permission to access any of the components registered in `query_state`.
    /// - `world` must be the same one used to initialize `query_state`.
...
impl<'w, 's, D: QueryData, F: QueryFilter, C: QueryCache, I: DoubleEndedIterator<Item = Entity>>
    QuerySortedManyIter<'w, 's, D, F, C, I>
{
    /// Get next result from the query
    #[inline(always)]
...
impl<'w, 's, D: ReadOnlyQueryData, F: QueryFilter, C: QueryCache, I: Iterator<Item = Entity>> Iterator
    for QuerySortedManyIter<'w, 's, D, F, C, I>
{
    type Item = D::Item<'w, 's>;
...
impl<'w, 's, D: ReadOnlyQueryData, F: QueryFilter, C: QueryCache, I: DoubleEndedIterator<Item = Entity>>
    DoubleEndedIterator for QuerySortedManyIter<'w, 's, D, F, C, I>
{
    #[inline(always)]
    fn next_back(&mut self) -> Option<Self::Item> {
        let entity = self.entity_iter.next_back()?;
...
impl<'w, 's, D: ReadOnlyQueryData, F: QueryFilter, C: QueryCache, I: ExactSizeIterator<Item = Entity>>
    ExactSizeIterator for QuerySortedManyIter<'w, 's, D, F, C, I>
{
}
...
pub struct QueryCombinationIter<'w, 's, D: QueryData, F: QueryFilter, C: QueryCache, const K: usize> {
    tables: &'w Tables,
    archetypes: &'w Archetypes,
    query_state: &'s QueryState<D, F, C>,
    cursors: [QueryIterationCursor<'w, 's, D, F, C>; K],
}

impl<'w, 's, D: QueryData, F: QueryFilter, C: QueryCache, const K: usize> QueryCombinationIter<'w, 's, D, F, C, K> {
    /// # Safety
    /// - `world` must have permission to access any of the components registered in `query_state`.
    /// - `world` must be the same one used to initialize `query_state`.
    pub(crate) unsafe fn new(
        ...
        let mut array: MaybeUninit<[QueryIterationCursor<'w, 's, D, F, C>; K]> = MaybeUninit::uninit();
        let ptr = array
            .as_mut_ptr()
            .cast::<QueryIterationCursor<'w, 's, D, F, C>>();
...
impl<'w, 's, D: ReadOnlyQueryData, F: QueryFilter, C: QueryCache, const K: usize> Iterator
    for QueryCombinationIter<'w, 's, D, F, C, K>
{
...
impl<'w, 's, D: ReadOnlyQueryData, F: QueryFilter, C: QueryCache, const K: usize> FusedIterator
    for QueryCombinationIter<'w, 's, D, F, C, K>
{
}

impl<'w, 's, D: QueryData, F: QueryFilter, C: QueryCache, const K: usize> Debug
    for QueryCombinationIter<'w, 's, D, F, C, K>
{
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
...
struct QueryIterationCursor<'w, 's, D: QueryData, F: QueryFilter, C: QueryCache> {
    ...
    current_row: u32,
    _cache: core::marker::PhantomData<C>,
}

impl<D: QueryData, F: QueryFilter, C: QueryCache> Clone for QueryIterationCursor<'_, '_, D, F, C> {
    fn clone(&self) -> Self {
        Self {
...
impl<'w, 's, D: QueryData, F: QueryFilter, C: QueryCache + 's> QueryIterationCursor<'w, 's, D, F, C> {
    ...
    #[inline]
    unsafe fn peek_last(&mut self, query_state: &'s QueryState<D, F, C>) -> Option<D::Item<'w, 's>> {
